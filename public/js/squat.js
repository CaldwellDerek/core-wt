    ...
    try {
        ...
        for (let max of userData){
            ...
            row.innerHTML = `
                ...
                <td style="background-color: black; color: white;">${max.squatMax}</td>
...
document.querySelector(".modal-confirm").addEventListener("click", async () => {
    const createMax = await fetch("/api/squat/create", {
        method: "POST",
        body: JSON.stringify({
            squatMax: Math.round((100 * document.querySelector("#weight").value) / (101.3 - (2.67123 * document.querySelector("#reps").value)))
        }),
        headers: {
